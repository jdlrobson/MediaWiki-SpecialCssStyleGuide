...
( function( w ) {
	...
	function renderRules( rules, options ) {
		...
		rules.forEach( function( ruleDef ) {
			var parent = container || document.body, isDirty, ruleParts = ruleDef.children, j;

			ruleParts.forEach( function( childDesc, j ) {
				var tagName, child = childDesc.el, el, el2,
					selectedNode = container.querySelectorAll( childDesc.selector ),
				...
				} else if ( selectedNode[0].tagName !== 'HTML' ) {
	...
	function parseSelector( nodeDescription ) {
		...
		try {
			...
			// FIXME: I don't trust that I parsed it correctly. If this logs something please open an issue!
			attrs = attrs || [];
			console.log( 'Problem creating tag:' + tagName + ' with class:' + className + ' and id:' + id + ' and attrs:' + attrs.join(',')  );
		}
	...
	function init( options ) {
		var stylesheet, cssRules, sheets = options.sheets || document.styleSheets;
	...
	if (document.querySelectorAll && String.prototype.trim ) {
		w.lsg = { init: init };
	} else {
